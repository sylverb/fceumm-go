		...
		*/
		...
		if (mapper == 0b100100)
		{
			if (get_bits(A, "11")) /* if (cpu_addr_in[11]) */
			{
				...
				case 0b100: /* 3'b100: begin // $C800, IRQ load8 */
...
#else
		WRAM = (uint8*)ahb_calloc(1, WRAM_SIZE);
#endif
...
#else
		SAVE_FLASH = (uint8*)ahb_calloc(1, SAVE_FLASH_SIZE);
#endif
...
#else
	CFI = (uint8*)ahb_calloc(1, sizeof(cfi_data) * 2);
#endif
